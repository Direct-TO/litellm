...
def test_zexapi_image_generation_preserves_url_and_base64(monkeypatch):
    ...
    assert config.get_complete_url(None, None, "gpt-image2", {}, {}) == "https://zexapi.com/v1/images/generations"
    assert config.validate_environment({}, "gpt-image2", [], {}, {}) == {
...
def test_zexapi_generation_request_is_openai_shaped():
    assert ZexAPIImageGenerationConfig().transform_image_generation_request(
        model="image2",
        prompt="poster",
        optional_params={"size": "720x1280", "response_format": "url"},
        litellm_params={},
        headers={},
    ) == {
        "model": "image2",
...
def test_zexapi_api_base_rejects_query_and_fragment():
    ...
    with pytest.raises(ValueError, match="query string or fragment"):
        config.get_complete_url("https://example.com/v1?tenant=a", None, "gpt-image2", {}, {})
    with pytest.raises(ValueError, match="query string or fragment"):
        config.get_complete_url("https://example.com/v1#fragment", None, "gpt-image2", {}, {})


def test_zexapi_public_image_generation_preserves_dual_output(respx_mock):
